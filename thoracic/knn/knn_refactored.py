...
def test_n_neighbors_sensitivity(X_train, X_test, Y_train, Y_test, logger, k_values=[1, 3, 5, 7, 9, 11, 15, 20, 25, 30, 50]):
    ...
    plt.ylabel("Score")
    plt.title("Sensitivity to n_neighbors (Multiple Metrics) - Thoracic")
    plt.legend()
    plt.grid(True)
...
def test_weights_sensitivity(X_train, X_test, Y_train, Y_test, logger, k=5):
    ...
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    plt.title(f'Weights Comparison (k={k}, All Metrics) - Thoracic')
...
def test_metric_sensitivity(X_train, X_test, Y_train, Y_test, logger, k=5):
    ...
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    plt.title(f'Distance Metric Comparison (k={k}, All Metrics) - Thoracic')
    plt.xticks(x + width/2, metrics_to_plot)
    plt.legend()
    plt.grid(True, axis='y', alpha=0.3)
    plt.tight_layout()
