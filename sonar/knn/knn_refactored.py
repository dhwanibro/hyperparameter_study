...
def load_and_preprocess(filepath):
    """Load sonar data and preprocess"""
...
def test_k_sensitivity(X_train, X_test, y_train, y_test, logger, k_values=list(range(1, 26))):
    ...
    plt.ylabel("Score")
    plt.title("KNN Sensitivity to K - Sonar")
    plt.legend()
    plt.grid(True)
...
def test_weights_comparison(X_train, X_test, y_train, y_test, logger, best_k=5):
    ...
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    plt.title('KNN Weights Comparison - Sonar')
...
def test_metric_comparison(X_train, X_test, y_train, y_test, logger, best_k=5):
    ...
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    plt.title('KNN Distance Metric Comparison - Sonar')
