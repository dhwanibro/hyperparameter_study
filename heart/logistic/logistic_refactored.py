...
def load_and_preprocess(filepath, target_col):
    ...
    if Y.min() == 1 and Y.max() == 2:
        print(f"Converting target from {{1,2}} to {{0,1}}")
        Y = Y.replace({1: 0, 2: 1})
...
def test_C_sensitivity(X_train, X_test, Y_train, Y_test, logger, C_values=[0.001, 0.01, 0.1, 1, 10, 100]):
    ...
    for C in C_values:
        # Logistic Regression is binary by default, solver='lbfgs' is the default and works well
        ...
        # Log to standardized logger (convert NumPy types)
    ...
    plt.ylabel("Score")
    plt.title("Sensitivity to C (Multiple Metrics) - Heart Disease")
    plt.legend()
    plt.grid(True)
...
def test_penalty_types(X_train, X_test, Y_train, Y_test, logger, C_fixed=1.0):
    """
    Test L1 vs L2 vs ElasticNet penalty with fixed C
    ...
    for config in penalty_configs:
        ...
        try:
            ...
            logger.log_experiment(
                ...
                param_value=penalty,
                metrics=convert_to_native_types(test_metrics),
                additional_info=convert_to_native_types({'C': C_fixed})
            )
    ...
    for i, penalty in enumerate(penalty_names):
    ...
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    plt.title(f'Penalty Comparison (C={C_fixed}, All Metrics) - Heart Disease')
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("Running penalty comparison (L1 vs L2 vs ElasticNet)...")
    print("="*60)
