...
def load_and_preprocess(filepath, target_col):
    """Load data and do preprocessing (no Label Encoding needed for features in this numerical dataset)"""
...
def test_C_sensitivity(X_train, X_test, Y_train, Y_test, C_values=[0.001, 0.01, 0.1, 1, 10, 100]):
    ...
    plt.ylabel("Score")
    plt.title("Sensitivity to C (Multiple Metrics) ")
    plt.legend()
    plt.grid(True)
...
def test_penalty_types(X_train, X_test, Y_train, Y_test, C_fixed=1.0):
    ...
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    plt.title(f'L1 vs L2 Penalty Comparison (C={C_fixed}, All Metrics) ')
...
if __name__ == "__main__":
    ...
    with open("logreg_sensitivity_results.txt", "w") as f:
        f.write("="*80 + "\n")
        f.write("LOGISTIC REGRESSION HYPERPARAMETER SENSITIVITY ANALYSIS - MAMOGRAM DATASET\n")
        f.write("="*80 + "\n\n")
        ...
        for C, metrics in c_results.items():
            test = metrics['test']
            # ONE-LINE FORMAT
            f.write(f"C = {C}: Test Metrics: Acc={test['accuracy']:.4f}, Prec={test['precision']:.4f}, Rec={test['recall']:.4f}, F1={test['f1']:.4f}, AUC={test['auc']:.4f}\n")
        ...
        for penalty, metrics in penalty_results.items():
            test = metrics['test']
            # ONE-LINE FORMAT
            f.write(f"{penalty.upper()}: Test Metrics: Acc={test['accuracy']:.4f}, Prec={test['precision']:.4f}, Rec={test['recall']:.4f}, F1={test['f1']:.4f}, AUC={test['auc']:.4f}\n")
    ...
    print("\nResults saved to: logreg_sensitivity_results.txt")
