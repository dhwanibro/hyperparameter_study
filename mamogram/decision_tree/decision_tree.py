...
if __name__ == "__main__":
    ...
    # ==========================================================
    # FILE WRITING SECTION (FIXED FOR HEATMAP PARSER)
    # ==========================================================
    with open("dt_sensitivity_results.txt", "w") as f:
        f.write("="*80 + "\n")
        f.write("DECISION TREE HYPERPARAMETER SENSITIVITY ANALYSIS - MAMOGRAM DATASET\n")
        f.write("="*80 + "\n\n")
        ...
        for depth, metrics in depth_results.items():
            test = metrics['test']
            # ONE-LINE FORMAT
            f.write(f"max_depth = {depth}: Test Metrics: Acc={test['accuracy']:.4f}, Prec={test['precision']:.4f}, Rec={test['recall']:.4f}, F1={test['f1']:.4f}, AUC={test['auc']:.4f}\n")
        ...
        for split, metrics in split_results.items():
            test = metrics['test']
            # ONE-LINE FORMAT
            f.write(f"min_samples_split = {split}: Test Metrics: Acc={test['accuracy']:.4f}, Prec={test['precision']:.4f}, Rec={test['recall']:.4f}, F1={test['f1']:.4f}, AUC={test['auc']:.4f}\n")
        ...
        for leaf, metrics in leaf_results.items():
            test = metrics['test']
            # ONE-LINE FORMAT
            f.write(f"min_samples_leaf = {leaf}: Test Metrics: Acc={test['accuracy']:.4f}, Prec={test['precision']:.4f}, Rec={test['recall']:.4f}, F1={test['f1']:.4f}, AUC={test['auc']:.4f}\n")
        ...
        for crit, metrics in criterion_results.items():
            test = metrics['test']
            # ONE-LINE FORMAT
            f.write(f"{crit}: Test Metrics: Acc={test['accuracy']:.4f}, Prec={test['precision']:.4f}, Rec={test['recall']:.4f}, F1={test['f1']:.4f}, AUC={test['auc']:.4f}\n")
