...
if __name__ == "__main__":
    ...
    with open("knn_sensitivity_results.txt", "w") as f:
        f.write("="*80 + "\n")
        f.write("KNN HYPERPARAMETER SENSITIVITY ANALYSIS - MAMOGRAM DATASET\n")
        f.write("="*80 + "\n\n")
        ...
        for k, metrics in k_results.items():
            train = metrics['train']
            test = metrics['test']
            # ONE-LINE FORMAT (CRITICAL FOR HEATMAP PARSER)
            f.write(f"k = {k}: Train Acc = {train['accuracy']:.4f}, Test Acc = {test['accuracy']:.4f}\n")
            # DETAILED METRICS (FOR YOUR REPORT)
            f.write(f"  Test Metrics: Prec={test['precision']:.4f}, Rec={test['recall']:.4f}, "
                   f"F1={test['f1']:.4f}, AUC={test['auc']:.4f}\n")
            f.write(f"  Train Metrics: Prec={train['precision']:.4f}, F1={train['f1']:.4f}\n\n")
        ...
        for weight, metrics in weights_results.items():
            ...
            test = metrics['test']
            # ONE-LINE FORMAT (CRITICAL FOR HEATMAP PARSER)
            f.write(f"{weight}: Train Acc = {train['accuracy']:.4f}, Test Acc = {test['accuracy']:.4f}\n")
            # DETAILED METRICS
            f.write(f"  Test Metrics: Prec={test['precision']:.4f}, Rec={test['recall']:.4f}, "
                   f"F1={test['f1']:.4f}, AUC={test['auc']:.4f}\n")
            f.write(f"  Train Metrics: Prec={train['precision']:.4f}, F1={train['f1']:.4f}\n\n")
        ...
        for dist_metric, metrics in metric_results.items():
            ...
            test = metrics['test']
            # ONE-LINE FORMAT (CRITICAL FOR HEATMAP PARSER)
            f.write(f"{dist_metric}: Train Acc = {train['accuracy']:.4f}, Test Acc = {test['accuracy']:.4f}\n")
            # DETAILED METRICS
            f.write(f"  Test Metrics: Prec={test['precision']:.4f}, Rec={test['recall']:.4f}, "
                   f"F1={test['f1']:.4f}, AUC={test['auc']:.4f}\n")
            f.write(f"  Train Metrics: Prec={train['precision']:.4f}, F1={train['f1']:.4f}\n\n")
